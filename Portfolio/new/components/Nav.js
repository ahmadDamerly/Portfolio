     ...
     mt-auto xl:right-[2%] z-50 top-0 w-full xl:w-16 xl:max-w-md xl:h-screen '>
      ...
      bg-white/10 backdrop-blur-sm text-3xl xl:text-xl xl:rounded-full '>
        ...
        {navData.map((link, index) => {
          return (<Link className={`${link.path === pathname && 'text-red-600'} relative flex items-center group hover:text-accent transition-all duration-300 `}
            ...
            <div className='absolute pr-14 right-0 hidden xl:group-hover:flex'>
              <div className='bg-white relative flex text-primary items-center p-[6px]
               rounded-[3px]'
              >
                <div className='text-[12px] text-black leading-none font-semibold capitalize'>
                  {link.name}
                </div>
